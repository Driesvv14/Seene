    ...
    @IBAction func Aanmelden(_ sender: UIButton) {
        ...
        FIRAuth.auth()?.signIn(withEmail: txt_email.text!, password: txt_wachtwoord.text!) { (user, error) in
            if error == nil && user != nil {
                ...
                let alert = UIAlertController(title: "Incorrecte aanmeldgegevens!", message: "Het emailadres of wachtwoord is incorrect. Gelieve het opnieuw te proberen aub.", preferredStyle: .alert)
